...
def test_model(model, sequence_length=None, verboose=False):
    ...
    if not np.isnan(Y.sum()):  # checks for a NaN anywhere
        ...
        acc = x.mean() * 100
        if verboose:
            print("the overall accuracy for sequence_length {0} was: {1}".format(
                sequence_length, x.mean()))
            print("per bit")
...
def train_model(model, epochs=10, min_size=5, max_size=20, callbacks=None, verboose=False):
    ...
    if verboose:
        for j in range(epochs):
            model.fit_generator(sample_generator, steps_per_epoch=10,
                                epochs=j+1, callbacks=callbacks, initial_epoch=j)
...
def lengthy_test(model, testrange=[5, 10, 20, 40, 80], epochs=100, verboose=True):
    ...
    model_saver = ModelCheckpoint(
        log_path + "/model.ckpt.{epoch:04d}.hdf5", monitor='loss', period=1)
